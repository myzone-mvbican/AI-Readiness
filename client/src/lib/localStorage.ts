 ...
 */
export function getGuestAssessmentData(surveyId: number): GuestAssessmentData | null {
  try {
    const allData = localStorage.getItem(STORAGE_KEYS.GUEST_ASSESSMENT_DATA);
    if (!allData) return null;
    
    const parsedData = JSON.parse(allData) as Record<number, GuestAssessmentData>;
 ...
 */
export function saveGuestAssessmentData(surveyId: number, data: Partial<GuestAssessmentData>): void {
  try {
    // Get existing data
    const allData = localStorage.getItem(STORAGE_KEYS.GUEST_ASSESSMENT_DATA);
    ...
    localStorage.setItem(STORAGE_KEYS.GUEST_ASSESSMENT_DATA, JSON.stringify(parsedData));
  } catch (e) {
 ...
 */
export function saveGuestAssessmentAnswers(surveyId: number, answers: AssessmentAnswer[], currentStep?: number): void {
...
export function saveGuestAssessmentResult(result: any): void {
  localStorage.setItem(STORAGE_KEYS.GUEST_ASSESSMENT_RESULT, JSON.stringify(result));
}

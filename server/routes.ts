...
import { teams, users, userTeams, surveys } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/csv-file/:filename", async (req, res) => {
    try {
      ...
      if (!fileContent) {
        console.log(
          "File not found in standard locations, checking default survey files",
        );
          ...
          for (const dir of directories) {
            if (fs.existsSync(dir)) {
              ...
              const surveyFiles = files.filter(
                (file) => file.startsWith("survey-") && file.endsWith(".csv"),
              );
  ...
  app.post("/api/auth/google/connect", authenticate, async (req, res) => {
    try {
      ...
      const updatedUser = await storage.connectGoogleAccount(
        userId,
        googleUserData.sub,
      );
    ...
    try {
      ...
      const questions = parsedData.data
        ...
        .map((row: any, index: number) => {
          return {
            id: parseInt(row["Question Number"], 10),
            category: row["Category"] || "",
            text: row["Question Summary"] || "",
            detail: row["Question Details"] || "",
          };
        });

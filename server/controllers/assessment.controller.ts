...
export class AssessmentController {
  ...
  static async regenerateRecommendations(req: AuthRequest, res: Response) {
    try {
      const assessmentId = parseInt(req.params.id);
      ...
      if (assessment.status !== "completed") {
        return res.status(400).json({
          success: false,
          message: "Assessment must be completed to regenerate recommendations",
        });
      }

      // Generate new AI recommendations
      const aiController = await import("./ai.controller");
      const mockRequest = {
        params: { id: assessmentId.toString() },
        user: req.user,
      } as AuthRequest;
      ...
      const mockResponse = {
        status: (code: number) => ({
          json: (data: any) => {
            aiResult = { statusCode: code, data };
          },
        }),
      } as any;

      await aiController.default.generateRecommendations(mockRequest, mockResponse);

      if (aiResult?.statusCode === 200 && aiResult?.data?.success) {
        res.status(200).json({
          success: true,
        ...
        res.status(500).json({
          success: false,
          message: aiResult?.data?.message || "Failed to regenerate recommendations",
        });
      }
    } catch (error) {

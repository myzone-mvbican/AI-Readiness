...
Return only the NAICS code that best represents this business.`;
  ...
  static async generateSuggestions(assessment: any, requestUserId?: number): Promise<AISuggestionsResult> {
    ...
    try {
      ...
      // Map answers to question text using actual survey questions
      const questions = assessmentWithSurvey.survey?.questions || [];
      
      if (!questions || questions.length === 0) {
        throw new InternalServerError("Survey questions not available for mapping answers");
      }
      
      // Create a map with both number and string keys to handle type mismatches
      const questionMap = new Map<string | number, string>();
      questions.forEach((q: any) => {
        if (q && q.question && q.id !== undefined && q.id !== null) {
          // Store original value (could be string or number)
          questionMap.set(q.id, q.question);
          
          // Also store as number if it can be converted
          const idAsNumber = Number(q.id);
          if (!isNaN(idAsNumber)) {
            questionMap.set(idAsNumber, q.question);
          }
          
          // Also store as string
          questionMap.set(String(q.id), q.question);
        }
      });

      const responses = answers.map((answer: any) => {
        if (!answer || answer.q === undefined || answer.q === null) {
          throw new InternalServerError(`Invalid answer structure: missing question reference`);
        }
        
        // Try lookup by question ID (handles both string and number types)
        let questionText = questionMap.get(answer.q);
        
        if (!questionText) {
          const answerQ = Number(answer.q);
          if (!isNaN(answerQ)) {
            questionText = questionMap.get(answerQ);
          }
        }
        
        if (!questionText) {
          questionText = questionMap.get(String(answer.q));
        }
        
        if (!questionText) {
          throw new InternalServerError(`Question with ID ${answer.q} not found in survey`);
        }
